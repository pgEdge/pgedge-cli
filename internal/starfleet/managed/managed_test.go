...
// TestReferenceIsShipped pins that the managed sub-reference is
// actually embedded. The starfleet module serves these bytes as a
// sub-reference, so an empty document would route an agent to nothing
// at all.
func TestReferenceIsShipped(t *testing.T) {
	if len(Reference) == 0 {
		t.Fatal("managed ships no sub-reference document")
	}
	...
	const title = "# pgedge starfleet managed"
